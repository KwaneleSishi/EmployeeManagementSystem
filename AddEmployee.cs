...
namespace EmployeeManagementSystem
{
    public partial class AddEmployee : UserControl
    {
        ...
        public void displayEmployeeData()
        {
            ...
            dataGridView1.DataSource = listData; // Reassign
        }
        ...
        private void addEmployee_addBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (connect.State == ConnectionState.Closed)
                {
                    ...
                    {
                        connect.Open();
                        string checkEmID = "SELECT COUNT(*) FROM employees WHERE id = @emID AND delete_date IS NULL";
                        ...
                        using (SqlCommand checkEm = new SqlCommand(checkEmID, connect))
                        {
                            ...
                            else
                            {
                                ...
                                string insertData = "INSERT INTO employees " +
                                    "(id, full_name, gender, contact_number" +
                                ...
                                string path = Path.Combine(@"C:\Users\HP\Documents\EmployeeManagementSystem\Directory"
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                DataGridViewRow row = dataGridView1.Rows[e.RowIndex];
                addEmployee_id.Text = row.Cells[0].Value.ToString(); // ID (index 0)
                addEmployee_fullName.Text = row.Cells[1].Value.ToString(); // Name (index 1)
                addEmployee_gender.Text = row.Cells[2].Value.ToString(); // Gender (index 2)
                addEmployee_phoneNum.Text = row.Cells[3].Value.ToString(); // Contact (index 3)
                addEmployee_position.Text = row.Cells[4].Value.ToString(); // Position (index 4)

                string imagePath = row.Cells[5].Value.ToString(); // Image (index 5)
                ...
                addEmployee_status.Text = row.Cells[7].Value.ToString(); // Status (index 7)
            }
        }
